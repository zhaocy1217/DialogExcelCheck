...
cn_special_castle_ids = set()
cn_special_above_below_ids = set()
cn_special_certificate_id = set()
cn_special_decorate_id = set()
...
class LocalizeChecker():
    ...
    async def warn_CN(self, local_path):
        self.error_usrs = [self.name_id.get('田明东')]
        return await self.check(
...
class NoticeManager():
    ...
    def send_file_notice(self, url:str, title: str, content: str = "", is_error: bool = False, error_usrs: set = None):
        ...
        if error_usrs:
            ...
            error_at = self.get_error_at(set())
